...
use App\Models\ReferenceStudentNumerationDevelopmentIndicator as Model;
use Validator;
class StudentNumerationDevelopmentIndicator extends Controller
{
    public function index(Request $request)
    {
      if($request->month) {
        $models = Model::where('reference_student_numeration_development_scope_id', $request->scope_id)->where('start_age_group' , '<=', $request->month)
        ->where('end_age_group', '>=', $request->month)->get();
      } else {
        $models = Model::orderBy('id')->with('ReferenceStudentNumerationDevelopmentScope:id,scope')->get();
      }
    ...
    public function store(Request $request)
    {
      ...
      $model = new Model();
      $model->reference_student_numeration_development_scope_id = $request->scope_id;
